...
// v3.1.1 compatibility fix: legacy main.js asks for a non-existent
// Electron path name "applicationSupport". Map it to appData so the
// existing path.join(app.getPath('applicationSupport'), APP) resolves to
// ~/Library/Application Support/薪连薪每日推进 on macOS.
  ...
  win.webContents.on('did-finish-load', () => {
    win.webContents.executeJavaScript(`
      (() => {
        ...
        mask.innerHTML = `
          <div class="xlx-modal" role="dialog" aria-modal="true" aria-labelledby="xlxTaskTitle">
            <div class="xlx-modal-head">
              <div><div class="xlx-modal-title" id="xlxTaskTitle">新增任务</div><div class="xlx-modal-sub">按结果创建任务，而不是只记一个待办标题</div></div>
              <button class="xlx-modal-close" type="button" data-close>×</button>
            </div>
            <form id="xlxTaskForm">
              <div class="xlx-form-grid">
              ...
              <div class="xlx-modal-actions"><button class="btn" type="button" data-close>取消</button><button class="btn primary" type="submit">保存任务</button></div>
            </form>
          </div>`;
        document.body.appendChild(mask);

        const close = () => mask.classList.remove('show');
        mask.addEventListener('click', e => { if (e.target === mask || e.target.matches('[data-close]')) close(); });
